 ...
 */
public class Papelera extends JFrame implements MouseListener, ActionListener {
    ...
    public Papelera() {
       ...
       // menu.setSize(1360, 400);
        ...
        this.cortar.setIcon(this.insertarImagen("src/explorador/imagenes/restore_page_40px.png", 70, 70));
        this.cortar.setRolloverIcon(this.insertarImagen("src/explorador/restore_page_40px.png", 65, 65));
        ...
        //menu.add(abrir);
       // menu.add(copiar);
        menu.add(cortar);   //restarurar es cortar y pegar .
       // menu.add(pegar);
        //menu.add(eliminar);
       // menu.add(editar);
    ...
    public void pintar() {
        NodeList listaNodos = Explorador.currentDir2.getChildNodes();
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == cortar) {
            ...
            if (tipoElemento.getText().equals("archivo")) {
                Explorador.elementoaPegar2 = Archivo.obtenerNodoHijoPapelera(nombreElemento.getText());
            } else {
                Explorador.elementoaPegar2 = Directorio.obtenerNodoHijoPapelera(nombreElemento.getText());
            }
            if (Explorador.elementoaPegar2.getAttribute("escritura").equals("Público") || Explorador.elementoaPegar2.getAttribute("dueño").equals(Explorador.currentUser2.getAttribute("nombre"))) {
                int id_removido = Integer.parseInt(Explorador.elementoaPegar2.getAttribute("id"));
                Elemento removido = new Elemento(id_removido, nombreElemento.getText(), tipoElemento.getText());
                removido.eliminarPapelera();
                pegar.setEnabled(true);
                Explorador.estadoPegar2 = true;
                System.out.println(Explorador.elementoaPegar2);
            if (Explorador.elementoaPegar2.getNodeName().equals("archivo")) {
                Archivo archivo = new Archivo(Explorador.getCurrentId2(), Explorador.elementoaPegar2.getAttribute("nombre"), "Archivo", "Lorem", true, true, "adm");
                archivo.agregar();
                this.dispose();
                //new Interfaz();
                new Papelera();
            } else {
                Directorio directorio = new Directorio(Explorador.getCurrentId2(), Explorador.elementoaPegar2.getAttribute("nombre"), "directorio", true, true, "Adm");
                directorio.agregar();
                this.dispose();
                //new Interfaz();
                 new Papelera();
